...
from mmdet3d.models.builder import build_neck, build_backbone
from mmdet3d.models.detectors.mvx_two_stage import MVXTwoStageDetector


@DETECTORS.register_module()
class BaselineMapRaster(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        if map_backbone is not None:
            self.map_backbone = build_backbone(map_backbone)
        else:
            self.map_backbone = self.img_backbone
        
        if map_embeddings is not None:
            self.use_map_embeddings = True
            self.map_embedding = build_neck(map_embeddings)
        else:
            self.use_map_embeddings = False
            self.map_embedding = None
        
        if map_neck is not None:
            self.map_neck = build_neck(map_neck)
        else:
            self.map_neck = self.img_neck
    ...
    def extract_map_feat(self, map_img, len_queue=None):
        ...
        if map_img is not None:
            ...
            if map_img.dim() == 5 and map_img.size(0) == 1:
                map_img.squeeze_()
            elif map_img.dim() == 5 and map_img.size(0) > 1:
                B, N, C, H, W = map_img.size()
                map_img = map_img.reshape(B * N, C, H, W)

            map_img_feats = self.map_backbone(map_img)

            if isinstance(map_img_feats, dict):
                map_img_feats = list(map_img_feats.values())
        else:
            return None
        
        if self.with_img_neck:
            map_img_feats = self.map_neck(map_img_feats)

        img_feats_reshaped = []
        for img_feat in map_img_feats:
            BN, C, H, W = img_feat.size()
            if len_queue is not None:
                img_feats_reshaped.append(img_feat.view(int(B/len_queue), len_queue, int(BN / B), C, H, W))
            else:
    ...
    def extract_map_embedding(self, map_img, len_queue=None):
        # map should have 2 channels: lane type category, number of lanes
        # map_img: B x H x W x 2
        map_embed = self.map_embedding(map_img)
        # permute to be: B x 1 x 2 x H x W
        map_embed = [me.permute(0, 3, 1, 2).unsqueeze(1) for me in map_embed]
        return map_embed    
    ...
    def forward_train(self,
        ...
        # gt_lc: list of tensors: batch, num_curves * 15
        prev_bev = None
        img_feats = self.extract_feat(img=img, img_metas=img_metas)  # list: 4, B x num_cam x channel_dim x H x W
        # check for img_feats shape

        # get bev features of SD map raster
        if self.use_map_embeddings:
            map_raster_feats = self.extract_map_embedding(map_img=map_raster)
        else:
            map_raster_feats = self.extract_map_feat(map_img=map_raster.permute(0, 3, 1, 2))
        bev_feats = self.bev_constructor(img_feats, img_metas, map_bev_feats=map_raster_feats, prev_bev=prev_bev)
    ...
    def forward_test(self, img_metas, img=None, map_raster=None, **kwargs):
        ...
        new_prev_bev, results_list = self.simple_test(
            img_metas, img, map_raster, prev_bev=None, **kwargs)
        return results_list

    def simple_test_pts(self, x, map_x, img_metas, img=None, prev_bev=None, rescale=False):
        """Test function"""
        batchsize = len(img_metas)

        bev_feats = self.bev_constructor(x, img_metas, map_bev_feats=map_x, prev_bev=prev_bev)
    ...
    def simple_test(self, img_metas, img=None, map_raster=None, prev_bev=None, rescale=False):
        ...
        new_prev_bev, bbox_results, lane_results, lclc_results, lcte_results = self.simple_test_pts(
            img_feats, map_raster_feats, img_metas, img, prev_bev, rescale=rescale)
