...
class CustomDefaultFormatBundle:
    ...
    def __call__(self, results):
        ...
        if 'map_raster' in results:
            results['map_raster'] = DC(to_tensor(results['map_raster']), stack=True)
        
        if 'map_graph' in results:
            # deciding between: num_polylines x max_num_vectors x 3 (points) or num_polylines x 15 (curves)
            results['map_graph'] = DC(to_tensor(results['map_graph']))  # keep as list of tensors
        if 'map_num_poly_pnts' in results:
            results['map_num_poly_pnts'] = DC(to_tensor(results['map_num_poly_pnts']))
